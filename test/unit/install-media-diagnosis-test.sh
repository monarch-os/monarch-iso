#!/bin/bash
#
# Unit tests for monarch-install-diagnose-media and the failure screen that
...
DIAGNOSE="$ROOT/configs/airootfs/usr/local/bin/monarch-install-diagnose-media"
DASHBOARD="$ROOT/configs/airootfs/usr/local/bin/monarch-install-dashboard"
...
diagnose() {
  MONARCH_OFFLINE_MIRROR="$mirror" MONARCH_TARGET_PACKAGE_CACHE="$TARGET_CACHE" \
    "$DIAGNOSE" "$@"
}
...
{
  echo '#!/bin/bash'
  echo "MONARCH_OFFLINE_MIRROR='$mirror' MONARCH_TARGET_PACKAGE_CACHE='$TARGET_CACHE' exec '$DIAGNOSE' \"\$@\""
} >"$stubs/monarch-install-diagnose-media"
chmod +x "$stubs/monarch-install-diagnose-media"
...
monarch_share="$work/share"
mkdir -p "$monarch_share"
...
{"current_phase": "Installing Arch + Monarch",
 "phases": [{"name": "Installing Arch + Monarch", "status": "failed",
             "error": "Pacstrap failed. See /var/log/archinstall.log"}]}
STATE
  : >"$screen"
  script -qefc "stty rows 40 cols 120; PATH='$stubs:$PATH' MONARCH_PATH='$monarch_share' MONARCH_UI_INTERACTIVE=no MONARCH_UI_FAILURE_ACTION=exit MONARCH_FAILURE_TAIL_LOG='$install_log' '$DASHBOARD' '$install_log' '$state_file' -- bash -c 'exit 1'" \
    "$screen" >/dev/null 2>&1
}
...
visible_screen | grep -qF "Monarch installation stopped" ||
