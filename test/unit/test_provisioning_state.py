...
class ContextDeferProvisioningTest(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {
            "MONARCH_INSTALL_CONFIG": str(self.dir / "user_configuration.json"),
            "MONARCH_INSTALL_CREDS": str(self.dir / "user_credentials.json"),
            "MONARCH_INSTALL_STATE_DIR": str(self.state_dir),
        }
    ...
    def from_env(self, **extra_env):
        ...
        with mock.patch.dict(os.environ, env, clear=False):
            for key in ("MONARCH_INSTALL_DEFER_PROVISIONING_FILE",):
                if key not in env:
    ...
    def base_config(self, defer_provisioning=None, disk_encryption=None):
        ...
        if defer_provisioning is not None:
            config["monarch_install"]["defer_provisioning"] = defer_provisioning
    ...
    def test_marker_file_arms_deferred_provisioning_without_credentials(self):
        ...
        marker.touch()
        ctx = self.from_env(MONARCH_INSTALL_DEFER_PROVISIONING_FILE=str(marker))
        self.assertTrue(ctx.defer_provisioning)
        self.assertTrue(ctx.monarch_install["defer_provisioning"])

    def test_absent_marker_file_is_not_deferred_provisioning(self):
        self.write_config(self.base_config())
        self.write_creds({"users": [{"username": "jeff"}]})
        ctx = self.from_env(MONARCH_INSTALL_DEFER_PROVISIONING_FILE=str(self.dir / "missing-defer_provisioning"))
...
def make_ctx(target, **overrides):
    defaults = dict(
        ...
        encrypt=False,
        username="",
        monarch_install={"mode": "full_disk", "defer_provisioning": True, "storage": {}},
...
class StageProvisioningStateTest(unittest.TestCase):
    ...
    def install_runtime_provisioning_support(self):
        service = self.target / "usr/share/monarch/install/provisioning/monarch-provision-owner.service"
        service.parent.mkdir(parents=True, exist_ok=True)
        service.write_text("[Unit]\n")
        setup_bin = self.target / "usr/bin/monarch-provision-owner"
        setup_bin.parent.mkdir(parents=True, exist_ok=True)
        runtime_bin = self.target / "usr/share/monarch/bin/monarch-provision-owner"
        runtime_bin.parent.mkdir(parents=True, exist_ok=True)
        runtime_bin.write_text("#!/bin/bash\n")
        setup_bin.symlink_to("/usr/share/monarch/bin/monarch-provision-owner")

    def provisioning_dir(self):
        return self.target / "var/lib/monarch/provisioning"
    ...
    def test_deferred_provisioning_arms_first_boot_setup(self):
        ...
        self.assertTrue((self.target / "etc/systemd/system/monarch-provision-owner.service").exists())
        link = self.target / "etc/systemd/system/multi-user.target.wants/monarch-provision-owner.service"
        self.assertTrue(link.is_symlink())
        self.assertEqual(
            os.readlink(link), "/etc/systemd/system/monarch-provision-owner.service"
        )
    ...
    def test_deferred_provisioning_encrypted_stages_luks_auto_unlock(self):
        ...
        keyfile = self.target / "etc/monarch/provisioning.key"
        self.assertEqual(keyfile.read_text(), "throwaway-secret")
        self.assertEqual(keyfile.stat().st_mode & 0o777, 0o600)

        cmdline = (self.target / "etc/limine-entry-tool.d/99-monarch-provisioning-unlock.conf").read_text()
        self.assertIn("cryptkey=rootfs:/etc/monarch/provisioning.key", cmdline)
        files = (self.target / "etc/mkinitcpio.conf.d/99-monarch-provisioning-key.conf").read_text()
        self.assertIn("FILES+=(/etc/monarch/provisioning.key)", files)

    def test_deferred_provisioning_pre_encrypted_without_passphrase_fails(self):
        self.install_runtime_provisioning_support()
        ctx = make_ctx(
            self.target,
            monarch_install={"mode": "protected", "defer_provisioning": True, "storage": {"luks_uuid": "abc"}},
        )
...
class ConfigureLoginDeferProvisioningTest(unittest.TestCase):
    ...
    def test_deferred_provisioning_login_leaves_user_state_to_first_boot(self):
        ...
        self.assertTrue((self.target / "etc/sddm.conf.d/99-monarch-login.conf").exists())
...
class ConfigureSshAccessDeferProvisioningTest(unittest.TestCase):
    ...
    def test_deferred_provisioning_stages_keys_for_first_boot(self):
        ...
        staged = self.target / "var/lib/monarch/provisioning/authorized_keys"
...
class CreateFactorySnapshotTest(unittest.TestCase):
    def setUp(self):
        ...
        self.findmnt = {
            ...
            "SOURCE": "/dev/mapper/monarch_root[/@]",
        }
        self.calls = []
    ...
    def test_snapshots_root_as_factory(self):
        ...
        self.assertIn(["mount", "-o", "subvolid=5", "/dev/mapper/monarch_root", top], self.calls)
        self.assertIn(
    ...
    def test_factory_snapshot_scrubs_provisioning_credentials(self):
        ...
        factory = top / "@factory"
        secrets = [
            factory / "var/lib/monarch/provisioning/authorized_keys",
            factory / "etc/tailscale/authkey",
            factory / "etc/monarch/provisioning.key",
        ]
        keep = factory / "var/lib/monarch/provisioning/groups"
