...
RELEASE="$ROOT/bin/monarch-iso-release"

grep -qF 'read -r -a BUILDER <<<"${BUILDER_CMD:-docker}"' "$MAKE"
grep -qF 'BUILDER=(sudo docker)' "$MAKE"
grep -qF '"${BUILDER[@]}" run' "$MAKE"
grep -qF 'MONARCH_PKGS_REPO="${MONARCH_PKGS_REPO:-https://github.com/monarch-os/monarch-pkgs.git}"' "$MAKE"
grep -qF 'validate_remote_ref "$MONARCH_INSTALLER_REPO" "$MONARCH_INSTALLER_REF"' "$MAKE"
grep -qF 'validate_remote_ref "$MONARCH_PKGS_REPO" "$MONARCH_PKGS_REF"' "$MAKE"
grep -qF 'INSTALLER_REF_SLUG="${MONARCH_INSTALLER_REF//\//-}"' "$MAKE"
grep -qF 'iso_ref="${latest_iso%.*}-$INSTALLER_REF_SLUG.iso"' "$MAKE"
grep -qF 'INSTALLER_REF_SLUG="${INSTALLER_REF//\//-}"' "$RELEASE"
grep -qF '"${MONARCH_INSTALLER_REPO:-https://github.com/monarch-os/monarch.git}" /monarch-source' "$BUILDER"
grep -qF '"${MONARCH_PKGS_REPO:-https://github.com/monarch-os/monarch-pkgs.git}" /monarch-pkgs' "$BUILDER"
