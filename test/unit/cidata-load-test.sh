#!/bin/bash
#
# Unit tests for monarch-cidata-load. The script takes a path prefix, so every
...
CIDATA_LOAD="$ROOT/configs/airootfs/usr/local/bin/monarch-cidata-load"
...
for file in user_configuration.json user_credentials.json user_full_name.txt user_email_address.txt authorized_keys tailscale_authkey; do
