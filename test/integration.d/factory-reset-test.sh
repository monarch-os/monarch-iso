#!/bin/bash
#
# Factory reset on a shared ESP: proves monarch-system-factory-reset hands a
# machine on without destroying a dual-boot setup (basecamp/monarch#6847).
...
# survive, the old Monarch identity (entry + directory) is gone, and a fresh
# Monarch entry exists. First boot asserts provisioning completes, the machine
...
# Linux installation cloned from the real Monarch entry so its shape is
  ...
  ssh_sudo "
    ...
    awk '/^\//{block=1} block' /boot/limine.conf |
      sed -e 's/$OLD_ID/$FOREIGN_ID/g' \
          -e 's|/EFI/Linux/monarch_|/EFI/Linux/foreign_|g' \
          -e 's|^/\(+\{0,1\}\)\([^/+]\)|/\1Foreign \2|' >/tmp/foreign-entries
    grep -q 'machine-id=$FOREIGN_ID' /tmp/foreign-entries
    for uki in /boot/EFI/Linux/monarch_*.efi; do
      cp \"\$uki\" \"\${uki/monarch_/foreign_}\"
    done
...
# A saved base image contains the runtime from the ISO that created it. During
# runtime development, optionally replace only the reset entry point so this
# scenario can validate the current source without rebuilding that base first.
install_runtime_under_test() {
  local source=${MONARCH_INTEGRATION_MONARCH_SOURCE:-}
  [[ -n $source ]] || return 0
  [[ -f $source/bin/monarch-system-factory-reset ]] || {
    echo "No reset command under MONARCH_INTEGRATION_MONARCH_SOURCE=$source" >&2
    return 1
  }
  ...
  scp -q -i "$SSH_KEY" -P "$SSH_PORT" \
    ...
    "$source/bin/monarch-system-factory-reset" \
    "$GUEST_USER@127.0.0.1:/tmp/monarch-system-factory-reset"
  scp -q -i "$SSH_KEY" -P "$SSH_PORT" \
    -o BatchMode=yes \
    -o IdentitiesOnly=yes \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
    "$source/default/shells/init" \
    "$GUEST_USER@127.0.0.1:/tmp/monarch-shell-init"
  ssh_sudo "install -m 755 /tmp/monarch-system-factory-reset /usr/bin/monarch-system-factory-reset"
  ssh_sudo '
    set -euo pipefail
    mountpoint -q /run/monarch-test-top || {
      mkdir -p /run/monarch-test-top
      mount -o subvolid=5 "$(findmnt -no SOURCE / | sed "s/\[.*//")" /run/monarch-test-top
    }
    btrfs property set -ts /run/monarch-test-top/@factory ro false
    install -m 644 /tmp/monarch-shell-init /run/monarch-test-top/@factory/usr/share/monarch/default/shells/init
    btrfs property set -ts /run/monarch-test-top/@factory ro true
    umount /run/monarch-test-top
  '
  log "Installed the runtime changes under test from $source"
}
...
start_reset() {
  ...
  ssh_guest "
    ...
    setsid script -qefc 'stty rows 35 cols 130; sudo -S -p \"\" monarch-system-factory-reset' /tmp/reset.out \
...
reset_phase() {
  ...
  log "Running monarch-system-factory-reset"
  start_reset
  ...
  check "foreign UKI survives staging" \
    ssh_sudo "test -f /boot/EFI/Linux/foreign_linux-cachyos.efi"
  check "old monarch entry is removed by staging" \
    ssh_sudo "! grep -q 'machine-id=$OLD_ID' /boot/limine.conf"
  check "old monarch boot directory is removed by staging" \
    ssh_sudo "! test -e /boot/$OLD_ID"
  check "a fresh monarch entry exists after staging" \
  ...
  ssh_sudo "cat /var/log/monarch-system-factory-reset.log" >"$RUN_DIR/factory-reset.log" || true
...
drive_first_boot() {
  ...
  # then boot the Monarch entry by hand so the rest of the flow still runs.
  if wait_for_screen "Cybersecurity" 120 2>/dev/null; then
    printf 'ok - %s\n' "the machine boots into first-boot setup unattended"
  else
    if ocr_screen | grep -qi "Monarch Bootloader"; then
      ...
      log "Selecting the Monarch entry manually to continue the run"
      press down; sleep 1; press down; sleep 1; press down; sleep 1
      press ret
      wait_for_screen "Cybersecurity" 600
    else
  ...
  while true; do
    ...
    if grep -qi "look right" <<<"$text"; then
      ...
      step="keyboard"; [[ -v answered[$step] ]] || {
        capture_console "success-firstboot-01-keyboard"
        # The product defaults to French, while type_text sends QEMU keycodes
        # using the US layout. Select the first entry (English US) explicitly.
        press home
        press ret
      }
      ...
      step="fullname"; [[ -v answered[$step] ]] || { type_text "Monarch Test"; press ret; }
    elif grep -qi "Email address" <<<"$text"; then
      step="email"; [[ -v answered[$step] ]] || { type_text "test@monarch.org"; capture_console "success-firstboot-02-form"; press ret; }
...
first_boot_phase() {
  ...
  while true; do
    rc=0
    ssh_guest "test -f /var/lib/monarch/provisioning/pending" 2>/dev/null || rc=$?
    (( rc == 0 || rc == 255 )) || break
    if ((waited >= 900)); then
  ...
  check "provisioning completed on first boot" \
    ssh_guest "! test -f /var/lib/monarch/provisioning/pending"
  ...
  check "old monarch identity never returns" \
    ssh_sudo "! grep -q 'machine-id=$OLD_ID' /boot/limine.conf"
