...
class InstallContext:
    ...
    monarch_install: dict[str, Any]
    defer_provisioning: bool = False

    target: Path = Path("/mnt")
    monarch_path: Path = Path("/usr/share/monarch")
    state_dir: Path = Path("/run/monarch-install")
    log_path: Path = Path("/var/log/monarch-install.log")
    target_log_path: Path = Path("/mnt/var/log/monarch-install.log")
    ...
    def from_env(cls) -> "InstallContext":
        config_str = os.environ.get("MONARCH_INSTALL_CONFIG")
        creds_str = os.environ.get("MONARCH_INSTALL_CREDS")
        if not config_str:
            raise RuntimeError("MONARCH_INSTALL_CONFIG must be set")

        config_path = Path(config_str)
        creds_path = Path(creds_str or "/root/user_credentials.json")
        user_configuration = json.loads(config_path.read_text())
        monarch_install = user_configuration.get("monarch_install") or _default_monarch_install(user_configuration)
        if monarch_install.get("schema_version") != 1:
            raise RuntimeError("monarch_install.schema_version must be 1")

        # Deferred provisioning: the whole system installs but user creation is deferred to
        # first boot. Selected by the configurator (monarch_install.defer_provisioning) or by
        # an `defer-provisioning` marker file on an autoinstall drive, which also replaces the
        # user_credentials.json requirement.
        defer_provisioning_marker = _optional_path(os.environ.get("MONARCH_INSTALL_DEFER_PROVISIONING_FILE"))
        defer_provisioning = bool(monarch_install.get("defer_provisioning")) or defer_provisioning_marker is not None
        monarch_install["defer_provisioning"] = defer_provisioning
        ...
        arch_configuration.pop("monarch_install", None)
        ...
        state_dir = Path(os.environ.get("MONARCH_INSTALL_STATE_DIR", "/run/monarch-install"))
        ...
        ctx = cls(
            ...
            monarch_install=monarch_install,
            defer_provisioning=defer_provisioning,
            state_dir=state_dir,
        )
        disk_config = user_configuration.get("disk_config", {})
        target_mount = monarch_install.get("target_mount") or disk_config.get("mountpoint")
        if target_mount:
    ...
    @property
    def mode(self) -> str:
        if mode := self.monarch_install.get("mode"):
            return mode
...
def _default_monarch_install(user_configuration: dict) -> dict[str, Any]:
    disk_config = user_configuration.get("disk_config", {})
