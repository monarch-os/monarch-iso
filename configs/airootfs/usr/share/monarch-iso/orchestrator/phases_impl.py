...
Phase ordering (full-disk and protected/pre-mounted):
    ...
    arch_install_system    → one archinstall flow for partition/mount-or-use,
                             base install, early Monarch packages, Limine setup,
                             useradd, runtime Monarch packages, fstab
    configure_hibernation  → root-owned swap/resume drop-ins
    run_system_finalizer   → arch-chroot root monarch-apply-system, including Snapper
    finalize_limine_boot   → final Limine config/UKI build after hardware drop-ins
    run_chroot_finalizer   → arch-chroot -u user monarch-provision-user
...
def _iso_ref() -> str:
    ...
    ref_file = Path("/root/monarch_iso_ref")
    if ref_file.exists():
        try:
...
#
# archinstall is patched in the wrapper (monarch-iso-install) BEFORE Python
...
def prepare_live(ctx: InstallContext) -> None:
    if ctx.is_protected:
        ...
        if disk:
            ...
            subprocess.run(["monarch-iso-cleanup-disk", disk], check=True)
...
# Reorders guided.py's perform_installation() so early Monarch packages install
# before user creation and before our Monarch-owned Limine setup copies files
...
def arch_install_system(ctx: InstallContext) -> None:
    ...
    a pre-mounted target, and Monarch derives boot/fstab details from that same
    input.
    """
    ...
    with arch.open_installer(config, ctx.target, silent=True) as installer:
        ...
        try:
            ...
            info("› installing Monarch runtime + monarch-base.packages")
...
def _configure_limine_boot(ctx: InstallContext, installer, config) -> None:
    ...
    if not arch.is_limine(config):
        raise RuntimeError("Monarch installs only support Limine bootloader setup")

    info("› installing bootloader (Limine)")
    if arch.is_pre_mount(config):
        _install_pre_mounted_limine(ctx)
    else:
        _install_limine_monarch(ctx, installer, config)
...
def _install_limine_monarch(ctx: InstallContext, installer, config) -> None:
...
def _install_pre_mounted_limine(ctx: InstallContext) -> None:
    ...
    if not esp_device:
        raise RuntimeError("monarch_install.storage.esp_device missing")
...
def _write_limine_pacman_hook(target: Path, hook_command: str) -> None:
    hook_contents = textwrap.dedent(
        ...
        [Action]
        Description = Deploying Monarch Limine after upgrade...
    ...
    (hooks_dir / "99-monarch-limine.hook").write_text(hook_contents)
...
def _limine_template(ctx: InstallContext, filename: str) -> Path:
    candidates = [
        ctx.target / "usr" / "share" / "monarch" / "install" / "assets" / "limine" / filename,
        ctx.target / "usr" / "share" / "monarch" / "default" / "limine" / filename,
        ctx.monarch_path / "install" / "assets" / "limine" / filename,
        ctx.monarch_path / "default" / "limine" / filename,
    ]
...
# installed kernel. monarch-apply-system's hardware scripts routinely install
# such packages (sof-firmware on Intel audio, nvidia-open-dkms and target-only
# hardware kernels), so the phase can pay for several full UKI
...
def _drop_archinstall_zram_conf(ctx: InstallContext) -> None:
    ...
    monarch-settings ships the tuning as a vendor drop-in at
    /usr/lib/systemd/zram-generator.conf.d/90-monarch.conf, which outranks the
...
def _mount_offline_package_cache(ctx: InstallContext) -> None:
    ...
    """
    source = Path("/var/cache/monarch/mirror/offline")
...
def _mask_mkinitcpio_pacman_hooks(
    ...
    for name in names:
        path = hooks_dir / name
        backup = hooks_dir / f"{name}.monarch-backup"
...
# ─────────────────────────────────────────────────────────────────────────────
# Install intent helpers: normalize the Monarch-specific part of the
...
def _boot_intent(ctx: InstallContext) -> dict:
    boot = dict(ctx.monarch_install.get("boot") or {})
...
def _storage_intent(ctx: InstallContext) -> dict:
    return dict(ctx.monarch_install.get("storage") or {})
...
def verify_protected_mounts(ctx: InstallContext) -> None:
    ...
    for key in ("esp_device", "root_device"):
        ...
        if not device:
            raise RuntimeError(f"free-space install: monarch_install.storage.{key} missing")
        if not Path(device).exists():
            raise RuntimeError(f"free-space install: {key} {device} does not exist")

    if not _is_mountpoint(target):
        raise RuntimeError(f"free-space install: {target} is not a mountpoint")
...
def _btrfs_root_device(ctx: InstallContext) -> str:
    ...
    if storage.get("luks_uuid"):
        return storage.get("root_mapper") or "/dev/mapper/monarch_root"
...
def _write_pre_mounted_fstab(ctx: InstallContext) -> None:
    ...
    lines = [
        "# /etc/fstab — generated by Monarch ISO",
...
def _write_pre_mounted_crypttab(ctx: InstallContext) -> None:
    ...
    crypttab.write_text(f"monarch_root  UUID={luks_uuid}  none  luks,discard\n")


def _build_pre_mounted_cmdline(ctx: InstallContext, btrfs_uuid: str) -> str:
    storage = _storage_intent(ctx)
    if storage.get("luks_uuid"):
        root_mapper = storage.get("root_mapper") or "/dev/mapper/monarch_root"
        return (
            f"cryptdevice=UUID={storage['luks_uuid']}:monarch_root "
...
def configure_hibernation(ctx: InstallContext) -> None:
    ...
    """
    setup = ctx.target / "usr" / "bin" / "monarch-hibernation-setup"
    if not setup.exists():
        _debug_log(ctx, "skipping hibernation: /usr/bin/monarch-hibernation-setup is not installed")
        return
    ...
    subprocess.run([
        ...
        "/usr/bin/monarch-hibernation-setup", "--force", "--no-rebuild",
    ], check=True)


def _install_debug_enabled() -> bool:
    return os.environ.get("MONARCH_INSTALL_DEBUG") == "1" or Path("/usr/share/monarch-iso/install-debug").exists()
...
#  3. arch-chroot as root → monarch-apply-system --first-install
#  4. arch-chroot as user → monarch-provision-user --first-install
# ─────────────────────────────────────────────────────────────────────────────
...
def _prepare_target_setup(ctx: InstallContext) -> None:
    ...
    bind_mounts = [
        ("/var/cache/monarch/mirror/offline", "/var/cache/monarch/mirror/offline"),
...
def _ensure_finalizer_log_started(ctx: InstallContext) -> tuple[str, int]:
    if "monarch_start_time" not in ctx.state:
        ctx.state["monarch_start_epoch"] = int(time.time())
        ctx.state["monarch_start_time"] = time.strftime("%Y-%m-%d %H:%M:%S")

    ctx.log_path.parent.mkdir(parents=True, exist_ok=True)
    ctx.log_path.touch(exist_ok=True)
    ctx.log_path.chmod(0o666)

    if not ctx.state.get("monarch_finalizer_header_written"):
        with ctx.log_path.open("a", encoding="utf-8") as log:
            log.write(f"=== Monarch Target Setup Started: {ctx.state['monarch_start_time']} ===\n")
        ctx.state["monarch_finalizer_header_written"] = True

    return ctx.state["monarch_start_time"], ctx.state["monarch_start_epoch"]
...
def _run_target_setup_command(ctx: InstallContext, cmd: list[str], *, user: str | None = None) -> None:
    ...
    monarch_start_time, monarch_start_epoch = _ensure_finalizer_log_started(ctx)

    target_log = ctx.target / "var" / "log" / "monarch-install.log"
    ...
    env_extras = [
        ...
        f"MONARCH_INSTALL_USER={ctx.username}",
        f"MONARCH_START_TIME={monarch_start_time}",
    ...
    if _install_debug_enabled():
        env_extras.append("MONARCH_INSTALL_DEBUG=1")
...
def run_system_finalizer(ctx: InstallContext) -> None:
    if ctx.defer_provisioning:
        cmd = ["/usr/bin/monarch-apply-system", "--defer-provisioning", "--first-install"]
    else:
        cmd = ["/usr/bin/monarch-apply-system", "--install-user", ctx.username, "--first-install"]
...
# setup (monarch-provision-owner) and factory reset (monarch-system-factory-reset) consume.
#
# Every install stashes the bundled Node tarball in /var/lib/monarch/provisioning/
...
PROVISION_STATE_DIR = "var/lib/monarch/provisioning"
PROVISION_KEYFILE = "etc/monarch/provisioning.key"
NODE_PACKAGES_DIR = Path("/opt/packages")


def _target_path_exists(target: Path, path: Path) -> bool:
    """Check a target file without resolving absolute package symlinks in the live ISO."""
    if not path.is_symlink():
        return path.exists()

    link_target = Path(os.readlink(path))
    if link_target.is_absolute():
        link_target = target / link_target.relative_to("/")
    else:
        link_target = path.parent / link_target
    return link_target.exists()
...
def stage_provisioning_state(ctx: InstallContext) -> None:
    ...
    service_src = ctx.target / "usr/share/monarch/install/provisioning/monarch-provision-owner.service"
    setup_bin = ctx.target / "usr/bin/monarch-provision-owner"
    if not service_src.exists() or not _target_path_exists(ctx.target, setup_bin):
        raise RuntimeError(
            "deferred-provisioning install requested, but the installed Monarch runtime does not ship "
            "first-boot setup (monarch-provision-owner + install/provisioning/monarch-provision-owner.service). "
    ...
    unit_dst = ctx.target / "etc/systemd/system/monarch-provision-owner.service"
    ...
    link = wants_dir / "monarch-provision-owner.service"
    link.unlink(missing_ok=True)
    link.symlink_to("/etc/systemd/system/monarch-provision-owner.service")
...
def _stage_provisioning_luks_unlock(ctx: InstallContext, provisioning_dir) -> None:
    ...
    cmdline_dropin = ctx.target / "etc/limine-entry-tool.d/99-monarch-provisioning-unlock.conf"
    cmdline_dropin.parent.mkdir(parents=True, exist_ok=True)
    cmdline_dropin.write_text(
        'KERNEL_CMDLINE[default]+=" cryptkey=rootfs:/etc/monarch/provisioning.key"\n'
    )

    files_dropin = ctx.target / "etc/mkinitcpio.conf.d/99-monarch-provisioning-key.conf"
...
def finalize_limine_boot(ctx: InstallContext) -> None:
    ...
    if "Monarch" not in limine_conf.read_text():
        raise RuntimeError(f"{limine_conf} has no Monarch entry")
...
# ─────────────────────────────────────────────────────────────────────────────
# configure_login: seed SDDM's last user/session for the password-only Monarch
...
def configure_login(ctx: InstallContext) -> None:
    ...
    (sddm_dir / "99-monarch-login.conf").write_text(
        "[Theme]\nCurrent=monarch\n\n"
    ...
    if ctx.encrypt and not ctx.defer_provisioning:
        autologin_conf.write_text(
            "[Autologin]\n"
            f"User={ctx.username}\n"
            "Session=monarch.desktop\n"
        )
    else:
        # deferred-provisioning installs have no user yet; monarch-provision-owner writes autologin
...
# keys an autoinstall drive supplied. A stock Monarch install ships openssh but
...
def configure_ssh_access(ctx: InstallContext) -> None:
    ...
    if ctx.defer_provisioning:
        ...
        # monarch-provision-owner to install once first boot creates the owner, and
...
[Service]
Type=simple
ExecStart=/usr/bin/sh -c 'until tailscale up --auth-key file:{TAILSCALE_AUTHKEY_TARGET}; do sleep 15; done; rm -f {TAILSCALE_AUTHKEY_TARGET}; systemctl disable monarch-tailscale-join.service'
...
def configure_tailscale(ctx: InstallContext) -> None:
    ...
    unit = ctx.target / "etc" / "systemd" / "system" / "monarch-tailscale-join.service"
    unit.parent.mkdir(parents=True, exist_ok=True)
    unit.write_text(TAILSCALE_JOIN_UNIT)
    subprocess.run(
        ["arch-chroot", str(ctx.target), "systemctl", "enable",
         "tailscaled.service", "monarch-tailscale-join.service"],
        check=True,
    )
...
def validate_boot(ctx: InstallContext) -> None:
    ...
    if "Monarch" not in limine_conf_text:
        raise RuntimeError(f"{limine_conf} has no Monarch entry")
    ...
    uki_prefix = _limine_setting(config_text, "CUSTOM_UKI_NAME", "monarch") or "monarch"
    ...
    if arch.has_uefi():
        ...
        # Hardware packages (monarch-hw-intel-ptl, …) can swap the kernel out
        ...
        installed_ukis = [uki for uki in ukis if uki.exists() and uki.stat().st_size]
        if not installed_ukis:
...
def _validate_encrypted_uki(uki: Path) -> None:
    try:
        ...
        raise RuntimeError(f"Could not inspect encrypted UKI {uki}: {detail}") from exc

    _assert_encrypted_uki_contents(str(uki), config, archive)


def _assert_encrypted_uki_contents(uki: str, config: str, archive: str) -> None:
    has_unlock_hook = re.search(r"\b(?:encrypt|sd-encrypt)\b", config) is not None
    has_cryptsetup = any(
        entry.rstrip() == "usr/bin/cryptsetup"
        for entry in archive.splitlines()
    )
    if not has_unlock_hook or not has_cryptsetup:
        raise RuntimeError(
            f"{uki} cannot unlock an encrypted root: "
            "its initramfs must contain encrypt or sd-encrypt and /usr/bin/cryptsetup"
        )
...
def _validate_provisioning_state(ctx: InstallContext) -> None:
    ...
    link = ctx.target / "etc/systemd/system/multi-user.target.wants/monarch-provision-owner.service"
    if not link.is_symlink():
        raise RuntimeError("deferred-provisioning install but monarch-provision-owner.service is not enabled")
...
def _assert_boot_hooks_restored(ctx: InstallContext) -> None:
    ...
    for name in TARGET_DEFERRED_BOOT_HOOKS:
        ...
        # limine-mkinitcpio-hook is a hard dependency of the Monarch runtime
...
# Taken at the end of every install, it is what makes monarch-system-factory-reset a
# true factory reset.
# ─────────────────────────────────────────────────────────────────────────────
...
FACTORY_SCRUB_PATHS = (
    "var/lib/monarch/provisioning/authorized_keys",
    "var/lib/monarch/provisioning/luks-key",
    "etc/monarch/provisioning.key",
    "etc/limine-entry-tool.d/99-monarch-provisioning-unlock.conf",
    "etc/mkinitcpio.conf.d/99-monarch-provisioning-key.conf",
    "etc/tailscale/authkey",
    "etc/systemd/system/monarch-tailscale-join.service",
    "etc/systemd/system/multi-user.target.wants/monarch-tailscale-join.service",
)
