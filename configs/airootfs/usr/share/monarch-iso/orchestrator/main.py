...
MONARCH_INSTALL_* environment variables before Python starts. This keeps
archinstall's import-time CLI parsing from seeing Monarch-specific flags.
"""
...
def build_phases(ctx: InstallContext):
    ...
    and the partition details Monarch needs for boot/fstab generation.
    """
    ...
    return [
        ...
        ("Installing CachyOS + Monarch",  arch_install_system),
...
def main() -> int:
    ...
    info(f"Installing Monarch for {who} → {ctx.target}")
